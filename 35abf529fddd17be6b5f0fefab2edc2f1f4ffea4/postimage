...
Perl_hek_dup(pTHX_ HEK *source, CLONE_PARAMS* param)
{
	...
	(void)share_hek_hek(shared);
        ...
        PERL_HASH(hash, HEK_KEY(source), HEK_LEN(source));
...
Perl_he_dup(pTHX_ const HE *e, bool shared, CLONE_PARAMS* param)
{
	...
	HeKEY_hek(ret) = shared;
        ...
        PERL_HASH(hash, HEK_KEY(source), HEK_LEN(source));
	HeKEY_hek(ret) = save_hek_flags(HeKEY(e), HeKLEN(e), 0/*HeHASH(e)*/,
                                        HeKFLAGS(e));
    }
...
/* Common code for hv_delete()/hv_exists()/hv_fetch()/hv_store().
   klen may be negative, denoting UTF8 then. */
void *
...
#ifdef DEBUGGING
/* return a temp. string for the hv actions */

STATIC const char*
S_action_name(pTHX_ const int action)
{
    static const char* names[] = {
        "DISABLE_UVAR_XKEY","DISCARD",
        "STORE","EXISTS","LVALUE","JUST_SV",
        "DELETE","EMPTY_HE" /* 0x80 */
    };
    SV* tmp = newSVpvn_flags("", 0, SVs_TEMP);
    int i;

    for (i=0; i<8; i++) {
        int j = 1<<i;
        if (action & j) {
            sv_catpvn(tmp, names[i], strlen(names[i]));
            sv_catpvs(tmp, ",");
        }
    }
    if (SvCUR(tmp)) {
        STRLEN cur = ((XPV*)SvANY(tmp))->xpv_cur--;
        SvPVX_mutable(tmp)[cur-1] = '\0';
    }
	       ...
	       int flags, int action, SV *val, U32 hash)
{
    ...
    if (keysv && (SvIsCOW_shared_hash(keysv))) {
        if (HvSHAREKEYS(hv)) {
            ...
            PERL_HASH(hash, HEK_KEY(keysv_hek), HEK_LEN(keysv_hek));
            DEBUG_H(PerlIO_printf(Perl_debug_log,
                        "HASH SHAREKEYS \t%s{%.*s SV}\n",
                         HvNAME_get(hv)?HvNAME_get(hv):"",
                         (int)HEK_LEN(keysv_hek), HEK_KEY(keysv_hek)));
        } else {
            PERL_HASH(hash, SvPVX_const(keysv), SvCUR(keysv));
            DEBUG_H(PerlIO_printf(Perl_debug_log,
                        "HASH \t\t%s{%.*s SV 0x%x}\n",
                         HvNAME_get(hv)?HvNAME_get(hv):"",
  ...
  not_found:
    ...
    if ( DO_HSPLIT(xhv) ) {
        ...
        DEBUG_H(PerlIO_printf(Perl_debug_log,
                    "HASH split %u\t%6u => %u\t%s\n",
                    (unsigned)XHvTOTALKEYS(xhv), (unsigned)xhv->xhv_max, (unsigned)(oldsize*2)-1,
                    HvNAME_get(hv)?HvNAME_get(hv):""));
    ...
    *return_action = HV_COMMON_MAGICAL_IGNORE;
    if (UNLIKELY(SvRMAGICAL(hv) && !(action & (HV_FETCH_ISSTORE|HV_FETCH_ISEXISTS)))) {
            ...
            || SvGMAGICAL((const SV *)hv))
        {
            ...
            HE *entry;
            DEBUG_H(PerlIO_printf(Perl_debug_log,
                        "HASH %6u\t%6u\ttied mg-fetch %s\t%s{%.*s}\n",
                        (unsigned)HvTOTALKEYS(hv), (unsigned)HvMAX(hv), action_name(action),
                        HvNAME_get(hv)?HvNAME_get(hv):"", (int)klen, key));
            ...
            *return_action = HV_COMMON_MAGICAL_RETURNS;
            ...
            I32 i;
            DEBUG_H(PerlIO_printf(Perl_debug_log,
                        "HASH %6u\t%6u\tcaseless mg-fetch %s\tENV{%.*s}\n",
                        HvTOTALKEYS(hv), HvMAX(hv), action_name(action), klen, key));
            for (i = 0; i < klen; ++i)
                if (isLOWER(key[i])) {
            ...
            || SvGMAGICAL((const SV *)hv))
        {
            ...
            SV * sv = sv_newmortal();
            DEBUG_H(PerlIO_printf(Perl_debug_log,
                        "HASH %6u\t%6u\ttied mg-exists %s\t%s{%.*s}\n",
                        (unsigned)HvTOTALKEYS(hv), (unsigned)HvMAX(hv), action_name(action),
                        HvNAME_get(hv)?HvNAME_get(hv):"", (int)klen, key));
        ...
        DEBUG_H(PerlIO_printf(Perl_debug_log,
                    "HASH %6u\t%6u\tmg-store '%c' %s\t%s{%.*s}\tcopy:%s, store:%s\n",
                    (unsigned)HvTOTALKEYS(hv), (unsigned)HvMAX(hv),
                    SvMAGIC(hv)->mg_type, action_name(action),
                    HvNAME_get(hv)?HvNAME_get(hv):"", (int)klen, key,
		   ...
		   int k_flags, I32 d_flags, U32 hash)
{
    ...
    for (; entry; oentry = &HeNEXT(entry), entry = *oentry) {
        ...
        if (memNE(HEK_KEY(hek),key,klen))	/* is this it? */
            continue;
	if ((HEK_FLAGS(hek) ^ masked_flags) & HVhek_UTF8)
	    continue;
	...
	while (entry) {				/* non-existent */
		...
		*oentry = HeNEXT(entry);
                ...
                if (aep[j] && PL_HASH_RAND_BITS_ENABLED) {
                    PL_hash_rand_bits+= ROTL32(hash, 17);
                    PL_hash_rand_bits= ROTL_UV(PL_hash_rand_bits,1);
                    if (PL_hash_rand_bits & 1) {
...
{
    U32 oldsize, oldkeys, deleted;
    PERL_ARGS_ASSERT_HV_STUDY;

    oldsize = HvMAX(hv) + 1;
    oldkeys = HvTOTALKEYS(hv);
    deleted = HvPLACEHOLDERS_get(hv);
    DEBUG_H(PerlIO_printf(Perl_debug_log,
...
Perl_hv_ksplit(pTHX_ HV *hv, U32 newmax)
{
    ...
    if (a) {
        DEBUG_H(PerlIO_printf(Perl_debug_log,
                    ...
                    (unsigned)XHvTOTALKEYS(xhv), (unsigned)xhv->xhv_max,
                    (unsigned)newsize-1, (unsigned)newmax,
                    HvNAME_get(hv)?HvNAME_get(hv):""));
        hsplit(hv, oldsize, newsize);
    } else {
...
Perl_newHVhv(pTHX_ HV *ohv)
{
	...
	for (i = 0; i <= hv_max; i++) {
		...
		HeKEY_hek(ent) = shared
                    ? share_hek_flags(HEK_KEY(hek), HEK_LEN(hek), hash, HEK_FLAGS(hek))
                    :  save_hek_flags(HEK_KEY(hek), HEK_LEN(hek), hash, HEK_FLAGS(hek));
		if (prev)
		    HeNEXT(prev) = ent;
		else
		    ents[i] = ent;
	...
	HE *entry;
	const U32 riter = HvRITER_get(ohv);
	HE * const eiter = HvEITER_get(ohv);
        U32 hv_keys = HvTOTALKEYS(ohv);
...
Perl_hv_copy_hints_hv(pTHX_ HV *const ohv)
{
	...
	while ((entry = hv_iternext_flags(ohv, 0))) {
		...
		(void)hv_common(hv, heksv, HeKEY(entry), HeKLEN(entry),
				 HeKFLAGS(entry), HV_FETCH_ISSTORE|HV_FETCH_JUST_SV, sv, 0);
...
S_clear_placeholders(pTHX_ HV *hv, U32 items)
{
	...
	while ((entry = *oentry)) {
		...
		*oentry = HeNEXT(entry);
		...
		if (--items == 0) {
		    /* Finished.  */
		    U32 placeholders = HvPLACEHOLDERS_get(hv);
...
SV*
Perl_hfree_next_entry(pTHX_ HV *hv, U32 *indexp)
{
	...
	if ((entry = iter->xhv_eiter)) {
            DEBUG_H(PerlIO_printf(Perl_debug_log, "HASH hfree iter [%u]\t%u %u DEL\t%s\n",
                                  (unsigned)*indexp, (unsigned)HvTOTALKEYS(hv),
                                  (unsigned)HvMAX(hv), HeKEY(entry)));
	...
	if (*indexp == orig_index) {
            DEBUG_H(PerlIO_printf(Perl_debug_log, "HASH hfree leaked key [%u] == %u\t%u %u\n",
                          (unsigned)*indexp,  (unsigned)orig_index,
                          (unsigned)HvTOTALKEYS(hv), (unsigned)HvMAX(hv)));
    ...
    DEBUG_H(PerlIO_printf(Perl_debug_log, "HASH hfree [%u] / %u\t%u %u clear\t{%s}\n",
                          (unsigned)*indexp,  (unsigned)orig_index,
                          (unsigned)HvTOTALKEYS(hv), (unsigned)HvMAX(hv), HeKEY(entry)));
...
void
Perl_hv_riter_set(pTHX_ HV *hv, U32 riter) {
    struct xpvhv_aux *iter;
...
void
Perl_unshare_hek(pTHX_ const HEK *hek)
{
    assert(hek);
...
S_share_hek_flags(pTHX_ const char *str, I32 len, U32 hash, int flags)
{
	...
	*head = entry;
	...
	if (!next) {			/* initial entry? */
            ...
            DEBUG_H(PerlIO_printf(Perl_debug_log,
                        "HASH split %6u\t%6u => %u\tstrtab\n",
                        (unsigned)XHvTOTALKEYS(xhv), (unsigned)xhv->xhv_max,
                        (unsigned)(oldsize*2)-1));
            hsplit(PL_strtab, oldsize, (U32)(oldsize * 2));
	}
    }
...
Perl_hv_placeholders_get(pTHX_ const HV *hv)
{
    ...
    return mg ? (U32)(mg->mg_len) : 0;
}

void
Perl_hv_placeholders_set(pTHX_ HV *hv, U32 ph)
{
    MAGIC * const mg = mg_find((const SV *)hv, PERL_MAGIC_rhash);
...
Perl_refcounted_he_chain_2hv(pTHX_ const struct refcounted_he *chain, U32 flags)
{
    ...
    placeholders = 0;
    while (chain) {
        HEK* hek = chain->refcounted_he_hek;
        HE *entry;
        HE **oentry;
        SV *value;
        U32 hash;
...
Perl_hv_assert(pTHX_ HV *hv)
{
    ...
    int real = 0;
    int bad = 0;
    const U32 riter = HvRITER_get(hv);
